...
exports.taskDomain = void 0;
const _errors_1 = require("@errors");
exports.taskDomain = {
    create: (input, routineId) => {
        ...
        if (!input.content || input.content.trim().length < 2) {
            throw new _errors_1.AppError("Content must have at least 2 characters", 400);
        }
        if (plannedEnd.getTime() <= plannedStart.getTime()) {
            throw new _errors_1.AppError("PlannedEnd must be after plannedStart", 400);
        }
        if (Number.isNaN(plannedStart.getTime()) ||
            Number.isNaN(plannedEnd.getTime())) {
            throw new _errors_1.AppError("Invalid date format", 400);
        }
